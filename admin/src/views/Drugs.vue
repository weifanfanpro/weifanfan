<template>
  <div class="page-container">
    ...
    >
      ...
      <template #filters>
        <input v-model="searchQuery" type="text" placeholder="搜索药品名称..." class="filter-input" />
      ...
      <template #cell-name="{ row }">
        <div class="drug-cell">
          <div class="drug-avatar" :style="{ background: getAvatarColor(row.name) }">{{ (row.name || '?')[0] }}</div>
          <div class="drug-info">
            <span class="drug-name">{{ row.name }}</span>
            <span class="drug-category" v-if="row.category">{{ row.category }}</span>
          </div>
        </div>
      </template>

      <template #cell-price="{ value }">
        <span class="price-tag" v-if="value">{{ value }}元</span>
        <span v-else class="text-muted">-</span>
      </template>

      <template #cell-stock="{ value }">
        <span :class="value < 10 ? 'stock-low' : ''">{{ value ?? '-' }}</span>
      </template>

      <template #cell-status="{ value }">
        <span class="status-badge" :class="value === 'enabled' ? 'active' : 'inactive'">{{ value === 'enabled' ? '正常' : '停用' }}</span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="action-btns">
          ...
          <button class="icon-btn" :title="row.status === 'enabled' ? '停用' : '启用'" @click="toggleStatus(row)">
            ...
            <div class="modal-body">
              <div class="form-grid">
                ...
                <div class="form-group">
                  <label>分类</label>
                  <input v-model="form.category" type="text" placeholder="如：感冒药" class="form-input" />
                </div>
                <div class="form-group">
                  <label>价格（元）</label>
                ...
                <div class="form-group full">
                  <label>适应症</label>
                  <input v-model="form.indication" type="text" placeholder="药品适应症" class="form-input" />
                </div>
                <div class="form-group full">
                  <label>用法用量</label>
                  <input v-model="form.dosageSummary" type="text" placeholder="用法用量说明" class="form-input" />
                </div>
                <div class="form-group">
                  <label>服药时间</label>
                  <select v-model="form.mealTiming" class="form-input">
                ...
                <div class="form-group">
                  <label>每日次数</label>
                  <input v-model.number="form.dailyFrequency" type="number" class="form-input" />
                </div>
                <div class="form-group full">
                  <label>注意事项</label>
                  <input v-model="form.warnings" type="text" placeholder="用药注意事项" class="form-input" />
                </div>
                <div class="form-group full">
                  <label>禁忌</label>
                  <input v-model="form.contraindications" type="text" placeholder="禁忌事项" class="form-input" />
                </div>
                <div class="form-group">
                  <label>生产厂家</label>
                ...
                <div class="form-group">
                  <label>批准文号</label>
                  <input v-model="form.approvalNumber" type="text" class="form-input" />
                </div>
              </div>
            </div>
...
const emptyForm = { name: '', category: '', indication: '', dosageSummary: '', mealTiming: '', dailyFrequency: 1, warnings: '', contraindications: '', manufacturer: '', approvalNumber: '', price: 0, stock: 0, status: 'enabled' }
const form = ref({ ...emptyForm })

const categories = computed(() => [...new Set(drugs.value.map(d => d.category).filter(Boolean))])

const summary = computed(() => [
  { label: '药品总数', value: drugs.value.length, color: '#0A6E5D' },
  { label: '正常', value: drugs.value.filter(d => d.status === 'enabled').length, color: '#10B981' },
  { label: '停用', value: drugs.value.filter(d => d.status === 'disabled').length, color: '#EF4444' },
  { label: '低库存', value: drugs.value.filter(d => d.stock < 10).length, color: '#F59E0B' }
])

const columns = [
  { key: 'name', label: '药品名称', width: '200px' },
  { key: 'price', label: '价格', width: '100px' },
  { key: 'stock', label: '库存', width: '80px' },
  { key: 'dailyFrequency', label: '每日次数', width: '90px' },
...
const filteredDrugs = computed(() =>
  drugs.value.filter(d => {
    if (searchQuery.value && !(d.name || '').includes(searchQuery.value)) return false
...
function getAvatarColor(name) {
  ...
  return colors[((name || '').charCodeAt(0) || 0) % colors.length] + '18'
}
...
async function loadDrugs() {
  try {
    drugs.value = await get('/admin/medicine-library/list')
  } catch (e) {
    console.error('加载药品库失败:', e)
  }
}
...
async function saveDrug() {
  ...
  try {
    await post('/admin/medicine-library', form.value)
...
async function toggleStatus(row) {
  ...
  if (!confirm(`确定${action}药品「${row.name}」？`)) return
  try {
    const newStatus = row.status === 'enabled' ? 'disabled' : 'enabled'
    await post(`/admin/medicine-library/${row.id}/status`, { status: newStatus })
    await loadDrugs()
  } catch (e) {
...
async function deleteDrug(row) {
  if (!confirm(`确定删除药品「${row.name}」？此操作不可恢复。`)) return
  try {
    await del(`/admin/medicine-library/${row.id}`)
    await loadDrugs()
  } catch (e) {
...
.drug-cell { display: flex; align-items: center; gap: 10px; }
.drug-avatar { width: 34px; height: 34px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 0.88rem; color: var(--text-primary); flex-shrink: 0; }
.drug-info { display: flex; flex-direction: column; gap: 2px; }
.drug-name { font-weight: 600; color: var(--text-primary); }
.drug-category { font-size: 0.75rem; color: var(--text-tertiary); }

.price-tag { display: inline-block; padding: 2px 8px; background: rgba(232,168,56,0.1); color: #D97706; border-radius: var(--radius-full); font-size: 0.78rem; font-weight: 500; }
